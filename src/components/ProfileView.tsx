...
export function ProfileView({ profile, status, onEdit, onDelete, onRefresh }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header bar */}
      <div className="flex items-center gap-3 px-4 py-2.5 bg-deep-void border-b border-hull-grey/50">
        ...
        {profile.username && (
          <span className="font-jetbrains text-xs text-chrome-silver/50">@{profile.username}</span>
        )}
        <span className="font-jetbrains text-[10px] text-hull-grey uppercase px-1.5 py-0.5 border border-hull-grey/30 rounded">
          {profile.connection_mode}
        </span>
        {!isManual && profile.provider && (
          <span className="font-jetbrains text-[10px] text-void-purple">
        ...
        {!status.connected ? (
          <button
            ...
            disabled={connecting}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-jetbrains font-semibold bg-bio-green/20 text-bio-green border border-bio-green/30 rounded hover:bg-bio-green/30 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-jetbrains font-semibold bg-claw-red/20 text-claw-red border border-claw-red/30 rounded hover:bg-claw-red/30 transition-colors"
          >
            <Square size={12} />
            Disconnect
          </button>
